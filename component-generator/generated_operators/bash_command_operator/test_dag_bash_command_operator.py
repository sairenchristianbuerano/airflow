"""
Test DAG for BashCommandOperator

Operator for executing bash commands with environment variables and working directory support

This DAG demonstrates the custom BashCommandOperator with runtime parameter support.
...
with DAG(
    dag_id='test_bash_command_operator',
    default_args=default_args,
    description='Operator for executing bash commands with environment variables and working directory support',
    schedule=None,  # Manual trigger only for testing
    catchup=False,
    tags=['test', 'custom-component', 'operator'],
    params={
        'bash_command': Param(default='echo Hello World', type='string', description='Bash command to run'),
    },
) as dag:

    # Create task using runtime parameters
    test_task = BashCommandOperator(
        task_id='test_bash_command_operator_task',
...
#   2. Find DAG: test_bash_command_operator
...
#
# Via CLI:
#   airflow dags test test_bash_command_operator $(date +%Y-%m-%d)
